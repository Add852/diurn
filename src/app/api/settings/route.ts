...
db.prepare(`UPDATE profiles SET
      ...
      // Media cache invalidation + fresh scan when the bucketing inputs change:
      // day offset (changes how timestamps map to dates), folder, or enable flag.
      const mediaNowEnabled = !!p.media_enabled && !!p.media_folder;
      const mediaWasEnabled = !!current.media_enabled && !!current.media_folder;
      const mediaInvalidated =
        (p.day_offset_hours !== undefined && p.day_offset_hours !== current.day_offset_hours) ||
        (mediaNowEnabled && (!mediaWasEnabled || (p.media_folder || "") !== current.media_folder));
      if (mediaInvalidated) {
        getDb().prepare("DELETE FROM media_cache WHERE profile_id = ?").run(p.id);
        if (mediaNowEnabled) {
          // Fire-and-forget: the viewer/chat polling picks up the scanning state.
          scanMediaFolder(p.media_folder!, p.id, p.timezone || current.timezone, p.day_offset_hours ?? current.day_offset_hours).catch(() => {});
        }
      }
    } catch (err: any) {
