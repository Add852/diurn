...
29. **Form interface + AI toggle (2026-09):** `asking_method` replaced by `input_method` (`form_single` | `form_each` | `chat_one_go` | `chat_one_by_one`, form default; existing chat users migrated). New `GET /api/form` builds context with zero LLM calls — no session, no greeting. `/chat` page dispatches on `input_method`: `FormContent` (new, form-content.tsx) or `ChatContent`. New profile cols: `ai_enabled` (master toggle — config kept, `aiAvailable()` in ai.ts treats off == unconfigured), `form_output` (`raw` verbatim | `ai` refined per answer_prompt; AI-off falls back to raw), `media_in_context` (media file paths into the AI prompt, off by default), `raw_context_enabled`+`raw_context_folder` (entry POST writes `{date}-context.json` atomically, from either interface — page passes its context object back). Entries POST accepts `{answers}` (form_each) / `{blob}` (form_single, AI-only, 400 when off) / `{session_id}` (chat); `settle()` transcript = chat messages or `user: {blob}`. Obsidian summaries no longer truncate (NOTE_BODY_CHARS 1400-char LLM input cap and 2000-char raw-content cap both removed — whole markdown read). Raw-context debug panel extracted to `raw-context-panel.tsx` (chat + form), visibility per-device via localStorage `diurn-show-context`, shown by default, toggle in Settings → General. Edge-case contract (settings hint): AI off → chat GET 400s "AI is not available"; form_single UI falls back to per-question inputs; `asked=false` questions become inputs (recorded asked=true — flag only meaningful with AI on, AI infers unasked); AI-output notes fall back to raw.
