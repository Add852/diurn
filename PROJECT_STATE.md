...
40. **Router-cache revisit fix (2026-09):** After item 39's staleTimes removal was shown to be a no-op (Next 14 defaults dynamic to 0 anyway — refetch on every revisit), next.config.mjs now sets experimental.staleTimes.dynamic = 30 so revisits serve the cached page shell instantly instead of a network round-trip (notably slow through the user's cloudflared tunnel) plus a loading.tsx skeleton flash plus full remount. The viewer's own data still goes through viewer-cache.ts SWR (30s window, invalidation on mutations) and media tiles through the SW CacheFirst route, so nothing serves stale data longer than intended. Confirmed the SW does not intercept page navigations (only / and /api/media/file are routed), so navigations hit the network by design; the router cache now hides that cost client-side.
39. **Viewer remount instant-restore (2026-09):** Leaving /viewer and returning re-ran every fetch (entries list, thumbnail batches, media pages) because remounts refetch by default and staleTimes.dynamic=0 had disabled the Next client router cache app-wide. Two fixes: (1) staleTimes override removed — settings freshness is unaffected because settings-client handleSave already calls router.refresh() after saves; (2) new src/lib/viewer-cache.ts — a module-scope SWR cache (30s revalidate window, single-flight dedupe, last-known-good survives refresh failure) that journal-view and media-view read through via cachedGet(), so a remount paints instantly from cache and revalidates in the background. Mutations invalidate it: EntryDialog PUT/DELETE, FormContent POST, and chat generateNote POST call invalidateViewerCache(). The media scan poll passes { fresh: true } to bypass the cache (must see scan progress). Also this commit: sharp added to webpack server externals, HEIC/HEIF MIME table entries.
38. **Thumb backfill + test-machine note (2026-09):** Fixed the gap where pre-existing libraries never got thumbnails: the scan only enqueued files whose mtime changed, so every unchanged photo silently fell back to full-res in all grids (the exact lag the user felt). The scan now re-enqueues unchanged image rows with no stored thumb (one-time backfill; undecodable files retry, self-healing if sharp gains the format). generateThumb writes tmp+rename so a partial WebP is never served. Chat/form context media now sends the thumb src (with fallback) since panels render thumb-size. Test-machine ops note: updates via `git pull` require a server restart to take effect (Node loads code at process start) and a rebuild (`npm run build` / `serve.sh`) — the observed missing `thumb` column was a pre-restart server, not a migration failure; `~/.diurn` data (data.db, thumbs) survives pulls fine and never needs removal.
